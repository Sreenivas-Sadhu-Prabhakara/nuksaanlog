...
func TestPostgresStore_RoundTrip(t *testing.T) {
	...
	in, _ := json.Marshal(Loss{Item: "milk", Qty: 5, CogsPerUnit: 40})
	saved, err := store.Save(Record{Input: in, Headline: 200, Label: "milk"})
	if err != nil {
		t.Fatalf("save: %v", err)
	}
	if saved.ID == 0 {
	...
	if len(items) == 0 || items[0].ID != saved.ID || items[0].Label != "milk" {
